...
%%% XXX ADDED IN THE VELOCITIES DUE TO THE ROTOR ROLLING AND PITCHING HERE!!!
% Inflow angle, (Pitch - inflow angle = Effective AoA)
        phi         =       atan2((lambda.*(omega.*R)+flow.vel_z),Vaz_perp+omega_mid-ui+flow.vel_perp);
% Resultant velocity at each element
        V_R         =       sqrt((lambda.*(omega.*R)+flow.vel_z).^2+(Vaz_perp+omega_mid-ui+flow.vel_perp).^2);
