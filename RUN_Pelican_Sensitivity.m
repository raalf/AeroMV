...
cg_y = [-8 -3 0 3 6.5 8 12 16 22 30]*0.001;
idxCGFACT = fullfact([length(cg_x),length(cg_y)]);

%% Moment of inertia
Ixx = 0.025322564860661*[-1.25 -1 0.5 0.95 1 1.1 1.25 2 5];
Iyy = 0.028041568149193*[-1.25 -1 0.5 0.95 1 1.1 1.25 2 5];
Izz = 0.029055618259337*[-1 0.5 1 2];
...
parfor q = 1:size(idxCGFACT,1)
    tic
    OVERWRITE = [];
    OVERWRITE.GEOM.VEH.vecCG = [cg_x(idxCGFACT(q,1)) cg_x(idxCGFACT(q,2))  (152.0153-118.7)*0.001];
